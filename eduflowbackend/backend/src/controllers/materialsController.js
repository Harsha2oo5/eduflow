  ...
  `,
    ...
    if (req.file && fs.existsSync(req.file.path)) {
      try {
        ...
        console.error(
          'Failed to delete file on processing failure:',
          cleanupErr
        );
      }
    }
...
const generateSummary = async (req, res) => {
  ...
  try {
    ...
    const truncatedText =
      material.raw_text.slice(0, 8000);
...
`;
    ...
    const summary =
      await callClaude(summaryPrompt);
...
module.exports = { uploadMaterial, generateSummary, generateQuiz, getClassMaterials, getMaterialById };
